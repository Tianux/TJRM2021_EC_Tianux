...
float max_output_speed=5500; //5500 60W 6500 80W 7000 100W
    ...
    else if(remoteState == KEY_REMOTE_STATE )
    {
        ...
        if(clearing_flag==0)
        {
                    ...
                    &&(RC_Ex_Ctl.key.v & KEY_PRESSED_OFFSET_D )!=KEY_PRESSED_OFFSET_D)
            {
                ...
                CMFollowVal=(rotate_speed+70*fabs(rotate_change_para))*rotate_speed_dir;
            ...
            else if((RC_Ex_Ctl.key.v & KEY_PRESSED_OFFSET_SHIFT )==KEY_PRESSED_OFFSET_SHIFT)
            {
                ...
                spin_x=spin_p*(keymove_x*cos(theta)-keymove_y*sin(theta));
                spin_y=spin_p*(keymove_x*sin(theta)+keymove_y*cos(theta));
