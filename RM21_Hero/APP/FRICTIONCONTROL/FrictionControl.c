...
void Friction_Key_Control(void){
	fric_key_count++;
	if (fric_key_count == 2000){
		last_fric_key = 0;
		fric_key_count = 0;
	}
	if (( last_fric_key == 0) && ((RC_Ex_Ctl.key.v & KEY_PRESSED_OFFSET_B )==KEY_PRESSED_OFFSET_B)){ //if firstkey == B
		last_fric_key = 1;
	}
	if (( last_fric_key == 1) && ((RC_Ex_Ctl.key.v & KEY_PRESSED_OFFSET_V )==KEY_PRESSED_OFFSET_V)){ // if secondkey == V
		last_fric_key = 2;
	}
	else{
		last_fric_key = 0;
	}
	if (( last_fric_key == 2) && ((RC_Ex_Ctl.key.v & KEY_PRESSED_OFFSET_B )==KEY_PRESSED_OFFSET_B)){ // if thirdkey == B
		last_fric_key = 3;
	}
	else{
		last_fric_key = 0;
